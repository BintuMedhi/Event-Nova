...
fs.createReadStream(CSV_FILE_PATH)
  .pipe(csv())
  .on('data', (row) => {
    if (row['Event Name'] && row['Event Name'].includes('Roses')) {
      row['Banner'] = '/assets/events/guns-n-roses-official.jpg';
    }
    records.push(row);
  })
  .on('end', () => {
    ...
    console.log('Successfully updated CSV for Guns N Roses banner');
  });
