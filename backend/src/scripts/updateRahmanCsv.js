...
fs.createReadStream(CSV_FILE_PATH)
  .pipe(csv())
  .on('data', (row) => {
    if (row['Event Name'] && row['Event Name'].includes('Rahman')) {
      row['Banner'] = '/assets/events/ar-rahman-official.jpg';
    }
    records.push(row);
  })
  .on('end', () => {
    ...
    console.log('Successfully updated CSV for A.R. Rahman banner');
  });
